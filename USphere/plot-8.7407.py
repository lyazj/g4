...
tree = uproot.open('USphere-8.740700.root:tree')
...
range_to_fit = np.arange(650 * len(time) // bins[-1], 1000 * len(time) // bins[-1], dtype='int')
...
plt.legend()
plt.tight_layout()
plt.savefig('NeutronGlobalTime-8.7407.pdf')
plt.savefig('NeutronGlobalTime-8.7407.png')
plt.close()
...
range_to_fit = np.arange(54 * len(time) // bins[-1], 120 * len(time) // bins[-1], dtype='int')
number_to_fit = number[range_to_fit]
...
plt.legend()
plt.tight_layout()
plt.savefig('NeutronGeneration-8.7407.pdf')
plt.savefig('NeutronGeneration-8.7407.png')
plt.close()
