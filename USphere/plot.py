...
plt.hist(ak.flatten(gt), np.linspace(0, 1200, 51), weights=np.ones_like(ak.flatten(gt)) / n)
...
plt.figure(figsize=(10, 8))
plt.hist(ak.flatten(gen), np.linspace(0, 300, 51), weights=np.ones_like(ak.flatten(gen)) / n)
